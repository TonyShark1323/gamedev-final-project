...
public class DoorControllerNoAnim : MonoBehaviour
{
    ...
    void OnEnable()
    {
        KeypadController.OnCodeEntered += HandleCodeEntered;
    }
    ...
    void Update()
    {
        if (inReach && Input.GetKeyDown(KeyCode.E))
        {
            if (isLocked)
            {
                // If the door is locked, show locked door text
                UIManager.Instance.HideTexts();
                UIManager.Instance.ShowDoorLockedText(true);
                // jammed.Play();
                return; // Skip attempting to open the door this frame
        ...
        RotateDoor();
        if (inReach && !isLocked) // Check if still in reach to update the text correctly
        {
            UIManager.Instance.ShowDoorText(!isOpen);
        }
    }
    ...
    private void HandleCodeEntered(string keypadDoorId, bool codeIsValid)
{
    if (doorId == keypadDoorId && codeIsValid)
    {
        isLocked = false; // Unlock the door when the correct code is entered
        // UIManager.Instance.ShowDoorText(!isOpen); // Optionally update text to indicate door can be opened
        // open.Play();
    }
}
